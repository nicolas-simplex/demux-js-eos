...
export class StateHistoryMessageEncoder {
  ...
  private async parseGetBlockResult(result: any) : Promise<StateHistoryBlock> {
    ...
    if(result.traces){
      ...
      let actionPromises : Promise<StateHistoryAction>[] = []
      result.traces.forEach((transactionTrace: any) => {
        let index = 0
        actionPromises.push(... transactionTrace.action_traces.map(async ([, actionTrace] : [any,any]) : Promise<StateHistoryAction> => {
          let [deserializedAction] = await this.api.deserializeActions([actionTrace.act])
          return {
  ...
  private async inflateBlock(rawBlock: any) {
    let buffer = this.newBufferFromHex(rawBlock)
    let block = Serialize.getType(this.types, "signed_block").deserialize(buffer)
    return block
  }
  ...
  private async inflateTraces(rawTraces: any) {
    ...
    let buffer = this.newBuffer()
    buffer.pushArray(new Uint8Array(rawTraces))
    let tracesCount = buffer.getVaruint32()
    for(let i = 0; i < tracesCount; i++){
      let trace = Serialize.getType(this.types, "transaction_trace").deserialize(buffer)[1]
      traces.push(trace)
    }
    return traces
  }
